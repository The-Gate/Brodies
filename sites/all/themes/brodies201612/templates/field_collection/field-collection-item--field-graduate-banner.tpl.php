...
if (strlen($overlay_title) > 0 or strlen($overlay_text) > 0 or strlen($overlay_button) > 0) {
  ...
  $overlay_output .= '<div class="cta-overlay-inner col-md-4">';

  if (strlen($overlay_title) > 0) {
    $overlay_output .= '<div class="overlay-title">' . $overlay_title . '<div class="title-icon"><img src="/' . drupal_get_path('theme', $GLOBALS['theme']) . '/images/title-icons/title-icon-graduates.png"></div></div>';
  }
  if (strlen($overlay_text) > 0) {
    $overlay_output .= '<div class="overlay-text">' . $overlay_text . '</div>';
  }
  if (strlen($overlay_button) > 0) {
    $overlay_output .= '<div class="overlay-button">' . $link_open . $overlay_button . $link_close . '</div>';
  }
  $overlay_output .= '</div>';
  $overlay_output .= '</div>';
}
...
?>
<div class="<?php print $classes; ?> cta-overlay"<?php print $attributes; ?>>
