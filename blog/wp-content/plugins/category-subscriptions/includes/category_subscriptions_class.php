 ...
 */
	...
	private function bulk_user_profile_cat_row(&$cat,&$subscriptions,&$user,&$output){
			...
			$output .= "<input type='checkbox' name='cscs' value='" . esc_attr($cat->cat_ID). "' id='cs_c_" .$cat->cat_ID."'" . (( $subscription_pref ) ? "checked='checked'" : ''). " > ";
			$output .= "<label for='cs_c_" .$cat->cat_ID ."'>". (($cat->depth > 1) ? str_repeat('&#8212;&nbsp;',$cat->depth - 1) : '') . htmlspecialchars($cat->cat_name) . "</label>";
			$output .= "<select name='csdt_" . $cat->cat_ID ."'>";
...
// Gate removed '&#8212;&nbsp;' from line 634 and added class 'sub' to category sub items
	public function user_profile_cat_row(&$cat,&$subscriptions){
		$subscription_pref = isset($subscriptions[$cat->cat_ID]) ? $subscriptions[$cat->cat_ID] : NULL;
?>

						<?php echo ($cat->depth > 1) ? str_repeat('<div class="sub">',$cat->depth - 1) : '' ?><input type="checkbox" name="category_subscription_categories[]" value="<?php echo esc_attr($cat->cat_ID); ?>" id="category_subscription_category_<?php echo $cat->cat_ID; ?>" <?php echo (( $subscription_pref ) ? 'checked="checked"' : '') ?> >
